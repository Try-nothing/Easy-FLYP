    ...
    def forward(self,
        ...
        if self.world_size > 1:
            all_image_features, all_text_features = gather_features(
                ...
                self.use_horovod) # gather features of all gpus

            if self.local_loss: # compute similarity between all text and all image features
        ...
        # create a positive and negative sample matrix according to the labels. If the labels of the i-the and j-th samples
        # are the same, the i-th row and j-th column of the matrix are 1, otherwise they are 0.
        if ground_labels is not None:
            ...
            if ignore: # pure CLIP loss
                I = torch.eye(equal_labels.shape[0],
                              device=device,
                              dtype=torch.float)
                labels = I - 100 * (equal_labels - I) # if the labels of the i-th and j-th samples are the same, the i-th row and j-th column of the matrix are -100, diagonal elements are -1, otherwise they are 0.

                image_logit_exp = torch.exp(
                    logits_per_image -
                    torch.max(logits_per_image, dim=1, keepdim=True).values) # normalize the logits_per_image
                text_logit_exp = torch.exp(
                    logits_per_text -
                    torch.max(logits_per_text, dim=1, keepdim=True).values)

                image_logit_exp = image_logit_exp * (labels != -100) # include diagonal elements and negative samples
                text_logit_exp = text_logit_exp * (labels != -100)

                image_logit_exp = torch.diagonal(image_logit_exp) / torch.sum(
                    image_logit_exp, dim=1) # normalize the similarity
